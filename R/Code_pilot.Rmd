...
# Data from main data sheet
mdata <- read_csv(here("data","ReviewMap_PFAS_main_pilot.csv"), skip = 0)
#dim(mdata) #8 rows 34 columns

# Data from locally saved and updated Excel file - Species_info sheet
...
# Data from locally saved and updated Excel file - PFAS_types sheet
...
# Data from locally saved and updated Excel file - PFAS_info sheet
pidata <- read_csv(here("data","PFAS_info.csv"), skip = 0) 
#dim(pidata) #35 rows 7 columns

# Bibliometric data of included papers (with references) from Scopus in .bib format:
bib_sco <- convert2df(here("data","scopus.bib"), dbsource = "scopus", format = "bibtex")
#dim(bib_sco) #8 rows 33 columns

# Critical apprisal data from locally saved and updated Excel file:
qdata <- read_csv(here("data", "ReviewMap_PFAS_quality_pilot.csv"), skip = 0) #loads AMSTAR2 assessment sheet
...
## Prepare - remove columns starting with "Checked" (info on data checking - these fields will be present in the final extraction sheet), also remove any other redundand columns
...
#str(mdata)
mdata %>% count(PFAS_focus, PFAS_one_many) %>% arrange(desc(n)) %>% filter(PFAS_focus != "NA") -> t_PFASfocus #filter out NA
...
tmesh <- str_to_lower(trimws(t_MESH$MeSH_terms)) #bring to lower case and remove ending and trailing whitespace from character strings, single character vector
tmesh <- sub("[\\*]", "", tmesh) #remove star character
...
t_MESH2 %>% select(MeSH_headings) %>% count(MeSH_headings) %>% arrange(desc(n)) %>% filter(MeSH_headings != "NA") ->  MeSH_headings_counts 
...
#str(mdata)
mdata %>% count(Human_animal_environment) %>% arrange(desc(n)) %>% filter(Human_animal_environment != "NA") %>% filter(Human_animal_environment != "")-> t_subject #filter out NA
...
# change to long format (one species per row - one or multple rows per study):
mdata %>% group_by(Human_animal_environment) %>% select(MeSH_terms) %>% separate_rows(MeSH_terms, sep = ";\\\n") %>% separate_rows(MeSH_terms, sep = "; ") -> t_MESHby
#str(t_MESHby)
...
t_MESHby2 %>% count(MeSH_qualifiers, Subject) %>% arrange(desc(n)) %>% filter(MeSH_qualifiers != "NA") %>% filter(MeSH_qualifiers != "") -> t_topic_subject #filter out NA
#t_topic_subject$MESH_headings <- factor(t_topic_subject$MESH_headings, levels = unique(t_topic_subject$MESH_headings[order(t_topic_subject$n, decreasing = FALSE)]))
...
#ggsave(here("plots","figure_PFAStype_subject.pdf"), width = 4, height = 8, units = "cm", scale = 2, device = cairo_pdf)
...
mspdata %>% count(Publication_year) %>% ggplot(aes(x = Publication_year, y = n)) + 
...
#ggsave(here("plots","figure_area_publ_year.pdf"), width = 4, height = , units = "cm", scale = 2, device = cairo_pdf)
...
ggplot(t_Raw_data, aes(x = Raw_data, y = n)) +
 theme_light() +
 theme(legend.position = "none", axis.title.y = element_blank()) +
 labs(title = expression("Raw data availability")) + #~bold(A.)~' Type and subject'
 coord_flip()  +
...
#pdf(file="./plots/figure_keywords_network.pdf", width = 8, height = 4, pointsize = 8, family = "Helvetica", fonts = c("Helvetica"))
#par(mfrow=c(1,1), mar=c(2,4,2,4))
NetMatrix_keywords <- biblioNetwork(bib_sco, analysis = "co-occurrences", network = "keywords", sep = ";")
NetMatrix_keywords_plot <- networkPlot(NetMatrix_keywords, normalize="association", n = 10, Title = "Keyword co-occurrences", type = "fruchterman", size.cex = TRUE, size = 10, remove.multiple = F, edgesize = 4, labelsize = 3, label.cex = TRUE, edges.min = 2, label.n = 9, alpha = 0.3) #adjust number and size of labels, as needed, etc.
#dev.off()
...
NetMatrix_authors_plot <- networkPlot(NetMatrix_authors,  n = 50, Title = "Author collaboration", type = "auto", size = 2, size.cex = TRUE, edgesize = 10, labelsize = 1.1) #note there are potentially some mistakes in authors initials
...
NetMatrix_coupling_plot <- networkPlot(NetMatrix_coupling, n = 20, Title = "Paper co-citation", type = "fruchterman", size=T, remove.multiple=FALSE, labelsize=0.8,edgesize = 5) #plot the network
...
histResults <- histNetwork(bib_sco, sep = ";") # "n" (number of most cited references to select) picked manually, larger numbers may give error
...
ggplot(t1, aes(x = Review_type_claimed, y = n)) +
 ...
 geom_col(aes(fill = Human_animal_environment), width = 0.7) + 
 theme_light() +
  theme(legend.position = "bottom", legend.box.background = element_rect(colour = "black"), legend.title = element_blank(),   legend.text=element_text(size = 4), axis.title.x = element_text(size = 10), axis.title.y = element_blank())
